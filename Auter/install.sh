...
cd /tmp && wget https://raw.githubusercontent.com/nanih98/open-source/master/Auter/auter.conf && sudo cp -ar auter.conf /etc/auter/
...
cd /tmp && wget https://raw.githubusercontent.com/nanih98/open-source/master/Auter/auter && sudo cp -ar auter /etc/cron.d

#Remove the files from /tmp , there are not necessary.
cd /tmp && rm -rvf auter auter.conf
